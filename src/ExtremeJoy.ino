...
void loop()
{
  ...
  uint16_t siglftx = lftx >> 8;
  siglftx <<= 2;
  uint16_t lwrlftx = lftx & 0xFF;
  lwrlftx <<= 2;
  Serial.printf("Lftx %x ", siglftx);
  Serial.printf("lftx %x ", lwrlftx);
  Serial.println();
  uint16_t siglfty = lfty >> 8;
  siglfty <<= 2;
  uint16_t lwrlfty = lfty & 0xFF;
  lwrlfty <<= 2;
  Serial.printf("Lfty %x ", siglfty);
  Serial.printf("Lfty %x ", lwrlfty);
  Serial.println();
  uint16_t sigrhtx = rhtx >> 8;
  uint16_t lwrrhtx = rhtx & 0xFF;
  sigrhtx <<= 2;
  lwrrhtx <<= 2;
  
  Serial.printf("Rhtx %x ", sigrhtx);
  Serial.printf("Rhtx %x ", lwrrhtx);
  Serial.println();
